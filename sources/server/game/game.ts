...
class Game
{
	 ...
	 */
	private _onPlayerRoll( currentPlayer: WebSocket, currentPlayerNumber: number, playerMove: string, lostMoveX: number, newMoveX: number, lostMoveY: number, newMoveY: number): void
	{
		
		let maxNumber: number = currentPlayerNumber;
		let colorMove: string = playerMove;
        let x1: number = lostMoveX;
        let x2: number = newMoveX;
        let y1: number = lostMoveY;
        let y2: number = newMoveY;
